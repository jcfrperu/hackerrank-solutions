...
func solution(lines []string, writer *bufio.Writer) {
	_, k := getInt(lines[0], 0), getInt(lines[0], 1)
...
func getInput() io.Reader {
	// from file
	return openFile("hackerrank/bill_division/testcases/003.input")
...
// getting values from list and remove read item
func readString(lines []string) (string, []string) {
	removed := lines[0]
	return removed, lines[1:]
}

func readInt(lines []string) (int, []string) {
	line, newLines := readString(lines)
	return parseInt(line), newLines
}

func readLong(lines []string) (int64, []string) {
	line, newLines := readString(lines)
	return parseLong(line), newLines
}

func readDouble(lines []string) (float64, []string) {
	line, newLines := readString(lines)
	return parseDouble(line), newLines
}
